...
| Hardware decoding, live rather than against a recording | works — the same comparison with `--hwaccel auto` reads 824 of 65535 against its own `screencap`, and 5284 with the channels exchanged. So VAAPI, the YUV420P the transfer asks the driver for, and the conversion after it all put the right picture out in a real session, which until now was only known from a recording fed through the decoder. The 824 and the 558 above are separate captures a few minutes apart and are not a comparison of the two decoders; what compares those is `whether_the_two_decoders_agree`, which has them equal byte for byte |
